...
class AppUtils {
  ...
  /// If [exception] is a `token_expired` [CustomGraphQlException],
  /// schedule a global "session expired" message on the [MessageProvider]
  /// (via the navigator key's current context). This makes the
  /// Consumer in `main.dart` pop up the re-authentication dialog and
  /// redirect to the passcode screen — regardless of who catches the
  /// exception or whether `handleServiceException` is called at all.
  static void _triggerSessionExpiredIfNeeded(Exception? exception) {
    if (exception is! CustomGraphQlException) return;
    if (exception.code != "token_expired") return;
